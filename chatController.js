...
exports.conversar = async (req, res) => {
    try {
        ...
        if (typeof pergunta !== "string" || !pergunta.trim()) {
            return res.status(400).json({ resposta: "A pergunta deve ser um texto nao vazio." });
        }
        ...
        const promptYoda = `Aja como o Mestre Yoda. Responda em Português: ${pergunta.trim()}`;
        ...
        // 5. Salva no banco
        await new Mensagem({ role: "user", parts: [{ text: pergunta.trim() }] }).save();
...
exports.limparHistorico = async (req, res) => {
    try {
        ...
        console.error(erro);
        res.status(500).json({ msg: "Nao foi possivel limpar o historico." });
    }
};